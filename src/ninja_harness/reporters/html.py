...
<body><div class="wrap">
  <h1>🥷 {escape(run.agent_name)} {header_badge}</h1>
  <div class="sub">Run <code>{escape(run.run_id)}</code> · {escape(run.task)}</div>
  {metrics}
  <div class="card"><h1>Trajectory ({len(run.steps)} steps)</h1>{steps_html}</div>
  {tools_section}
  {handoffs_section}
  <div class="card final"><h1>Final Output</h1><pre>{escape(run.final_output)}</pre></div>
